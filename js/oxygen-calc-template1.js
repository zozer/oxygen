...
function getinputs(mode="import") {
	...
	try {
		...
		parser.add_argument('-RR',                functionault=12.0,        type=float,    help='bpm')
		parser.add_argument('-VT',                functionault=0.475,        type=float,    help='l')
		parser.add_argument('-VD',                functionault=0.11,        type=float,    help='l')
		parser.add_argument('-fio2',            functionault=0.21,        type=float,    help='fraction')
		parser.add_argument('-alt',                functionault=0,            type=float,    help='m')
		parser.add_argument('-CO',                functionault=6.5,        type=float,    help='l/min')
		parser.add_argument('-pulm_shunt',        functionault=0.02,        type=float,    help='fraction')
		parser.add_argument('-DmO2',            functionault=300,        type=float,    help='mlO2/min/kPa')
		parser.add_argument('-Vc',                functionault=0.075,        type=float,    help='l')
		parser.add_argument('-Hb',                functionault=150,        type=float,    help='g/l')
		parser.add_argument('-BE',                functionault=0,            type=float,    help='mEq/l')
		parser.add_argument('-DPG',                functionault=0.00465,    type=float,    help='M')
		parser.add_argument('-MCHC',            functionault=340,        type=float,    help='g/l')
		parser.add_argument('-VO2',                functionault=0.25,        type=float,    help='l/min')
		parser.add_argument('-Temp',             functionault=309.65,        type=float,    help='K') // 36.5 C = 309.65 K
		parser.add_argument('-tissue_shunt',     functionault=0.05,        type=float,    help='fraction')
		parser.add_argument('-RQ',                functionault=0.8,        type=float,    help='fraction')
		parser.add_argument('-ecmosites',        functionault=[],         action='append', help='use this to append as many values as you want')
		parser.add_argument('-Qecmo',            functionault=0,            type=float,    help='l/min')
		parser.add_argument('-hetindex',        functionault=0,            type=float,    help='heterogeneity index')
		parser.add_argument('-maxruns',            functionault=100,        type=int,    help='Normally 100')
		parser.add_argument('-debugging',        functionault=False,        type=bool,    help='')

		parser.add_argument('-outfile',        functionault='null',    help='')
		parser.set_functionaults()
...
//----------------------------------------------------------------------------
function fix_permissions(this_path) {
	os.system("/bin/chmod 755 %s"%(this_path))

function check_dir(this_dir) {
...
function calculatedconstants() {
	...
	trueVO2 = VO2 // user-functionined VO2 is an aspiration...
	//------------------------------------------------------------------------
...
function SnO2_2(CnO2_local, P50_SnO2) {
	...
	return optimize.brentq(SnO2_2_null,1e-15,(1-1e-15),args=(CnO2_local, P50_SnO2),rtol=toleranceoferror_so2, maxiter=15)
//----------------------------------------------------------------------------
...
function CnCO2_HbBound(PnCO2, PnO2, pH) {
...
function CnCO2_1(pH, PnCO2, PnO2) {
...
//----------------------------------------------------------------------------
function vanSlyke_1(pH, BE, SnO2) {
...
//----------------------------------------------------------------------------
function acidbase_1_null(pH,BE,PnCO2,PnO2) {
	'''returns 0'''
...
function acidbase_1(BE,PnCO2,PnO2) {
	'''returns PLASMA pH from PARTIAL PRESSURE '''
	try {
		return optimize.brentq(acidbase_1_null,1,14,args=(BE,PnCO2,PnO2),rtol=toleranceoferror_ph, maxiter=15) // pH units
	catch {
		print(BE, PnCO2, PnO2)
...
function acidbase_2_null(pH,BE,CnCO2,CnO2) {
	'''returns 0'''
	...
	beta = 9.5+1.63*Hb*1e-1
	return ((BE*1e3 - 0.2*Hb*1e-1*(1-SnO2))*zeta**-1 - beta*(pH-7.4) + StdBicarb*1e3)*1e-3 - HH_1(PnCO2, pH) // null
function acidbase_2(BE,CnCO2,CnO2) {
	'''returns PLASMA pH from CONTENT '''
	try {
		return optimize.brentq(acidbase_2_null,1,14,args=(BE,CnCO2,CnO2),rtol=toleranceoferror_ph, maxiter=15) // pH units
	catch {
		print(BE,CnCO2,CnO2)
...
//-------------------------------------------------------------------------------
function dxdt(inputs, t, VQ, DmO2_ivp, Vc_ivp, CvO2_local, CvCO2_local) {
	CcO2_local = inputs[0]
...
//-------------------------------------------------------------------------------
function dxdt_organ(inputs, t, DmO2_ivp, vol_b, CinputO2, CinputCO2, Q, compound, Qd, CdiO2, CdiCO2) {
...
function runorgan(dtype, preorganCnO2, preorganCnCO2, heter_stat=1e-200, ncomp=20) {
	if (dtype=="multicompartment_lung") {
		...
		cdf_boundaries = np.linspace(0,1-1e-6,ncomp) // functionining compartment boundaries. Starting at 0 and running to 0.999999. I.e. the top 1e-6th of the distribution is ignored.
...
//-------------------------------------------------------------------------------
function updatebloodgascontents() {
...
//-------------------------------------------------------------------------------
function updatepartialpressures(compartments = ['c','a','t','v']) {
	for (i in compartments) {
		//set floor to prevent error in acidbase_2 
		if (getattr(module, "C%sO2"%i) < 0) {
			setattr(module, "C%sO2"%i, 0.1)
		if (getattr(module, "C%sCO2"%i) < 0) {
			setattr(module, "C%sCO2"%i, 0.1) 
...
function timecheck(p,t) {
	global timelimit
	tc = timeit.functionault_timer() - t
...
function check_completion() {
	...
	if (debugging and verbose) {
		...
		for (key, value in sorted(diff.items(), key=lambda item: (item[1],item[0]), reverse=True)[:5]) {
			print(key, value,)
		print()
...
function formatoutput(method="full", label='') {
	if (CtO2 <= 0) {
		fail("Ct02<0")
	if (offline_mode) {
		if (method=="part") {
		...
		if (method=="full") {
			outstring = ''
			outstring += 'settings | ' + ' | '.join(["%s:%s"%(name,value) for name,value in variables.items()])
...
function circulate_once(iterationnumber=0) {
	...
	if (mbc) {
		print("mass balance check[c]: VO2:", CO*(1-pulm_shunt)*(CcO2-CvO2)*10,  "VCO2:", CO*(1-pulm_shunt)*(CvCO2-CcCO2)*10)
	// ======== arteries ========
	...
	if (mbc) {
		print("mass balance check[a]: VO2:", CO*(CaO2-CvO2)*10,  "VCO2:", CO*(CvCO2-CaCO2)*10)
	// ========= ECMO ==========
	ecmodelivery=0
	...
	if (mbc) {
		print("mass balance check[ae]: VO2:", CO*(CaO2-CvO2)*10,  "VCO2:", CO*(CvCO2-CaCO2)*10)
	// ========= tissues ==========
	...
	if (mbc) {
		print("mass balance check[t]: VO2:", Qtissue*(CaO2-CtO2)*10,  "VCO2:", Qtissue*(CtCO2-CaCO2)*10)
	// ========= veins ==========
	CvO2 = (CtO2*Qtissue + CaO2*CO*tissue_shunt)/CO
	CvCO2 = (CtCO2*Qtissue + CaCO2*CO*tissue_shunt)/CO
	updatepartialpressures(compartments = ['v'])
	if (mbc) {
		print("mass balance check[v]: VO2:", CO*(CaO2-CvO2)*10,  "VCO2:", CO*(CvCO2-CaCO2)*10)
	// ========= set VO2 ==========
	criticalOER = 0.94 // unrealistic maximum
	criticalDO2 = (CO*CaO2*10)*criticalOER/1000 // mlsO2/min
	if (criticalDO2 < VO2) {
		//then set trueVO2 = criticalDO2, but prevent oscillation {
	...
	if ("venacava" in ecmosites) {
		...
		updatepartialpressures(compartments = ['v'])
	if (mbc) {
		print("mass balance check[ve]: VO2:", CO*(CaO2-CvO2)*10,  "VCO2:", CO*(CvCO2-CaCO2)*10)
